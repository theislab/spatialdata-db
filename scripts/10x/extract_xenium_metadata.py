...
import time
import numpy as np
from metadata_utils import find_html_file, extract_rendered_html, is_number_string, safe_int, extract_summary_fields_from_html
...
def main(input_csv, output_csv, failed_csv):
    ...
    for _, row in tqdm(xenium_df.iterrows(), desc="Scraping Dataset Urls (test)"):
        uid = row['uid']
        url = row['Dataset Url']
        try:
            ...
            html_metadata = extract_summary_fields_from_html(rendered_html, XENIUM_FIELD_MAP, XENIUM_FALLBACKS)
    ...
    df_extracted = pd.DataFrame(extracted).rename(columns={'Panel type': 'Panel Type', 'Panel name': "Panel", "Number of target genes": "Number of Target Genes", "Number of cells detected": "Number of Cells", "Median transcripts per cell": "Median Transcripts per Cell", "Region area (µm²)": "Region Area", "Total cell area (µm²)": "Total Cell Area", "Number of custom target genes (RNA)": "Number of Add-on Genes"})
    df_extracted['Number of Target Genes'] = df_extracted.apply(fill_missing_target_genes, axis=1)
    df_extracted = df_extracted.drop(columns=['Number of predesigned target genes (RNA)'])
    ...
    df_extracted = df_extracted.drop(columns=['Slide ID', 'Cassette name'])
    ...
    df_extracted['Panel Type'] = df_extracted['Panel Type'].replace({'Add_on': 'Custom Add-on', "Add on": "Custom Add-on", "Add-on Custom": "Custom Add-on"})
    df_extracted['Panel'] = df_extracted['Panel'].replace({'Xenium Human Immuno-Oncology Add-on B Gene Expression': 'Xenium Human Immuno-Oncology + Add-on',
